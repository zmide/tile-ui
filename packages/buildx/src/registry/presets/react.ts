...
function buildReactCore(workspaceRoot: string) {
  ...
  return [
    ...
    '',
    stripImport(
      stripImport(buttonLogic, "import type { ButtonVariant, ButtonSize } from './button.types';\n"),
...
export function createReactRegistryConfig(input: {
  workspaceRoot: string;
  outDir: string;
}): Omit<RegistryBuildOptions, 'manifest'> {
  return {
    framework: 'react',
        ...
        return [
          {
            ...
            content: buildReactCore(workspaceRoot),
          },
          {
